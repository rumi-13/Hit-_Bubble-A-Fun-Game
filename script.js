...
function createBubble() {
    ...
    for (let row = 0; row < numRows; row++) {
        for (let col = 0; col < numColumns; col++) {
            ...
            let leftPosition = col * bubbleSize + col; // Adjusted for spacing
            let topPosition = row * bubbleSize + row; // Adjusted for spacing

            // Check if there is enough space at the top (considering bubble height)
            if (topPosition + bubbleSize <= containerHeight) {
